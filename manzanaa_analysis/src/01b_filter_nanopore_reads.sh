...
do
  ...
  unmapped_out_path=../../minibinders_data/minibinders_outputs/$experiment_name/alignments/unmapped/${fname%_R[12]_001.fastq*}
	...
	cat ${fastp_out_path}_out_ngmerged.assembled.fastq | seqkit stats > ${seqkit_stats_out_path}_filtering_stats.txt
	cat ${fastp_out_path}_out_ngmerged.assembled.fastq | seqkit amplicon -m 0 -P -s -F $flank_f -R $flank_r -r 21:-21 --bed > ${seqkit_filtered_read_variants_out_path}_variants.txt
    ...
    fastqc ${unmapped_out_path}_unmapped.fastq -o ../../minibinders_data/minibinders_outputs/$experiment_name/alignments/unmapped/
